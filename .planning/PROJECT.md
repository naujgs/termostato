...
**v1.2 in progress — Phase 6 complete 2026-05-15**

Phase 6 result: all 4 Mach kernel APIs (host_statistics, host_statistics64, task_info, task_threads) return KERN_SUCCESS under free Apple ID sideload on iOS 18. No graceful fallback needed. Phase 7 wires all 4 into the dashboard.

- Xcode 26.4.1 + Swift 6.3 strict concurrency + SwiftUI + Swift Charts
- `ProcessInfo.thermalState` polling (10s) + `thermalStateDidChangeNotification` observer
...
- [ ] Implement confirmed-accessible Mach APIs (CPU + memory) in dashboard UI — *Phase 7*
...
---
*Last updated: 2026-05-15 — Phase 6 complete, all Mach APIs confirmed accessible*
