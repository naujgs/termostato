...
# CoreWatch — v1 Requirements

## v1 Requirements

### Display

- [x] **DISP-01**: App displays current thermal state (Nominal / Fair / Serious / Critical) prominently with distinct color coding for each level
- [x] **DISP-02**: App displays a session history step-chart of thermal state changes since the app was opened (in-memory only, not persisted)
...
- [x] **INST-01**: App targets iOS 18+ and is installable directly via Xcode sideload onto the owner's iPhone — no App Store submission required
