...
|----------|----------|--------|---------|
| `CoreWatch/CoreWatch/SystemMetrics.swift` | Mach API probe logic with four probe functions and verdict classification | VERIFIED | 287 lines. Contains `enum APIVerdict`, `struct MachProbeResult`, `@Observable @MainActor final class SystemMetricsProbe` with all 4 probe methods, `runProbeSequence()`, `cancelProbe()`, majority verdict logic, and `[CoreWatch]` console logging. No TemperatureViewModel reference (D-01 isolation confirmed). |
...
- cpu_ticks user values (10410413, 10414312, 10418146) appeared in `[CoreWatch] host_statistics CPU:` console lines at ~10-second intervals
- Page counts (free ~5610-5732, active ~40646-41338) appeared in `[CoreWatch] host_statistics64 Memory:` lines
- Resident size ~79 MB appeared in `[CoreWatch] task_info Memory:` lines
- Thread count 5-6 and cpu% 0.0-2.0% appeared in `[CoreWatch] task_threads CPU:` lines
